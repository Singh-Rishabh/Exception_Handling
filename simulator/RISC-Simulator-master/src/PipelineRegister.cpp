...
PipelineRegister::PipelineRegister(bool pipe): bubble(pipe), PC(pipe), nPC(pipe), instruction_word(pipe), branchTarget(pipe), A(pipe), B(pipe), operand2(pipe), aluResult(pipe), ldResult(pipe),  flag(pipe), 	isSt(pipe), isLd(pipe), isBeq(pipe), isBgt(pipe), isRet(pipe), isImmediate(pipe), isWb(pipe), isUbranch(pipe), isCall(pipe), isAdd(pipe), isSub(pipe), isCmp(pipe), isMul(pipe), isDiv(pipe), isMod(pipe), isLsl(pipe), isLsr(pipe), isAsr(pipe), isOr(pipe), isAnd(pipe), isNot(pipe), isMov(pipe),isMovz(pipe),isRetz(pipe), mark(false){} 
...
void PipelineRegister::WriteBubble(bool to_bubble) {
	...
	if (to_bubble) {
		...
		isNot.Write(false);
		isMov.Write(false);
		isMovz.Write(false);//add
		isRetz.Write(false);//add
	}
	else {
		bubble.Write(false);
	}

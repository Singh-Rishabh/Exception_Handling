...
Core::Core(): MEM(MEM_CAPACITY), if_of(pipeline), of_ex(pipeline), ex_ma(pipeline), ma_rw(pipeline), PC(true), nPC(false) {
...
void Core::reset_proc()
{
	...
	oldFlags = 0; //add
	flags= 0;//add
	//flags.Write(0); //add flags will be 1 when eq and 2 when gt;
...
void Core::run_simplesim(){
	...
	int counter = 0;
	ifstream ifile("interrupt.txt");
	if (ifile.fail()){
			//cout<<"Cant access INTERRUPT FILE "<<endl;
	}
	int cycleNo= 0;
	int pid  = 0;
	vector<int> vec;
	while(ifile>>cycleNo>>pid){
		vec.push_back(cycleNo);
	}int j=0;
...
void Core::fetch_begin() {
	...
	//unsigned int temp_nPC = nPC.Read(); //add
	if (checkValidPC(PC.Read())){
		...
		//if_of.nPC.Write(temp_nPC); //add

	}
	
	else {
		if_of.mark.Write(rohit);
		if_of.WriteBubble(true);
		rohit = false;
	}
...
void Core::decode() {
	...
	bool temp_isNot;
	bool temp_isMov;
	bool temp_isMovz; //add
	bool temp_isRetz; //add
	...
	if( !(opcode5 == 1 || ( opcode5 == 0 && opcode3 == 1 && opcode1 == 1 && ( opcode4 == 1 || opcode2 == 0) ) ) 	|| 	(opcode5 == 1 && opcode4 == 0 && opcode3 == 0 && opcode2 == 1 && opcode1 == 1)  ){
		temp_isWb = true;
		//pprint(2)<<"isWb ";
	}else if(opcode5 == 1 && opcode4 == 0 && opcode3 == 1 && opcode2 == 0 && opcode1 == 1){
		temp_isWb = true;
	}
	else{
		temp_isWb = false;
	...
	else if( opcode5 == 1 && opcode4 == 0 && (	(opcode3 == 0 && opcode2 == 1) || (opcode3 == 1 && opcode2 == 0 && opcode1 == 0) ) 	){
		temp_isUbranch = true;
		//pprint(2)<<"isUbranch ";
	}
	else{
	...
	if(opcode5 == 1 && opcode4 == 0 && opcode3 == 1 && opcode2 == 0 && opcode1 == 1){//add
		temp_isMovz = true;
		//pprint(2)<<"isMov ";
	}
	else{ 
	...
	if(opcode5 == 1 && opcode4 == 0 && opcode3 == 1 && opcode2 == 1 && opcode1 == 0){ //add
		temp_isRetz = true;
	...
	if (inst_bitset(temp_instruction_word, 27, 27) == 1){//need to change
	...
	}else if (temp_isRetz){ //add
		temp_operand1 = oldPC ;
	     	// store it in the new set of restistor
		//pprint(2)<<"Operand1: "<<dec<<temp_operand1<<" (Read from ra OR R15)"<<endl;
		fprint(1)<<"; "<<"Retz inst"<<" = 0x"<<endl;
		//cout << "PC READING " <<PC.Read()<<endl;
	}
	else{
	...
	of_ex.isMovz.Write(temp_isMovz); //add
	of_ex.isRetz.Write(temp_isRetz); //add
...
void Core::execute() {
	...
	unsigned int temp_nPC ;//add
	unsigned int temp_flags; //= of_ex.flag.Read();  //add
	...
	bool temp_isMovz = of_ex.isMovz.Read();//add
	bool temp_isRetz = of_ex.isRetz.Read();//add


	
	//////////   Branch Unit  ///////////
	//pprint(2)<<"*** Branch Unit"<<endl;
	if (temp_isRet || temp_isRetz){ //add
		branchPC = temp_A ;
	...
	if (temp_isUbranch){
		//cout << "ddshvsalkv asvasgiuahgaiulhsiulgsiulghiuldghd ggibe"<<endl;
		isBranchTaken = true;
	...
	if (temp_isDiv){
	    if (temp_B == 0){//add
	       causeRegister = 2;
	       m = true;
	       // cout << "Dividing by zero "<< causeRegister.Read()<<endl;
	       //cout << "Dividing by zero "<< causeRegister<<endl;
        }
        else{
	...
	if( temp_isLd || temp_isSt){
		if (temp_aluResult < 0 || temp_aluResult > (MEM_CAPACITY - sizeof(unsigned int)) ){
			if(temp_isSt) causeRegister = 5; 
			else if(temp_isLd) causeRegister = 4;
			m = true;	
		}	
	}
	...
	if ( m || isInterrupt){
		...
		//cout << "Cause register greater than zero"<<endl;
		if (m){
			isException = true;
			cout << "Exception occured CYCLE NO : "<< countC << " exception id "<< causeRegister<<endl;
			oldPC = temp_PC + 4;//add
			oldFlags = flags;
			oldSP = R[14] ;
			...
			//branchPC = 0x4;
			//isBranchTaken = true;
			//causeRegister.Write(0);
			causeRegister = 0;
			if_of.WriteBubble(true);
			of_ex.WriteBubble(true);
			ex_ma.WriteBubble(true);
		
			if_of.mark.Write(false);
			of_ex.mark.Write(false);
			//ex_ma.mark.Write(false);
			m = false;
			return;
		}else {
			//isException = true;
	...
	ex_ma.isMovz.Write(temp_isMovz); //add
	ex_ma.isRetz.Write(temp_isRetz); //add
...
void Core::mem_access() {
	...
	//unsigned int temp_nPC = ex_ma.nPC.Read(); //add
	...
	//unsigned int temp_flag = ex_ma.flag.Read(); //add
	...
	bool temp_isMovz = ex_ma.isMovz.Read();//add
	bool temp_isRetz = ex_ma.isRetz.Read();//add
	...
	ma_rw.isMovz.Write(temp_isMovz);//add
	ma_rw.isRetz.Write(temp_isRetz);//add
...
void Core::write_back() {
	...
	//unsigned int temp_nPC = ma_rw.nPC.Read(); //add
	...
	//unsigned int temp_flags = ma_rw.flag.Read(); //add
	...
	bool temp_isMovz = ma_rw.isMovz.Read(); //add
	bool temp_isRetz = ma_rw.isRetz.Read(); //add
...
bool Core::checkValidPC(unsigned int testPC){
	...
	if (inst_bitset(testInst,28, 32) < 23)  {// add
		return true;
	}
	causeRegister = 1;
	rohit = true;
...
bool Core::check_data_conflict(PipelineRegister& A, PipelineRegister& B){
	...
	if (B_opcode5 == 1 && B_opcode4 == 0 && B_opcode3 == 1 && B_opcode2 == 1 && B_opcode1 == 0){ //add
		return false;	//B is retz
	}
...
string Core::disassemble (unsigned int inst_word){
	...
	if (opcode5 == 1 && opcode4 == 0 && opcode3 == 1 && opcode2 == 0 && opcode1 == 1){
		//inst = "MOVZ" + modifier + " " + registerstring(rd);
		//if (isImmediate){
		//	inst += ", " + hexstring(imm);
		//}
		//else {
